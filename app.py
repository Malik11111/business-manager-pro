...
def api_graphes_summary():
    ...
    formation_stats = []
    for tf in types_f:
        recs = FormationRecord.query.filter_by(etab_id=etab.id, type_formation_id=tf.id).all()
        recs_by_pid = {r.personnel_id: r for r in recs}
        ok = bientot = expire = vide = 0
        for pid in pers_ids:
            r = recs_by_pid.get(pid)
        ...
        formation_stats.append({
            'nom': tf.nom,
            'ok': ok, 'bientot': bientot, 'expire': expire, 'vide': vide,
